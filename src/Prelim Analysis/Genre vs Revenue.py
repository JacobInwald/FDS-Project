...
genre_data = genre_data_clean
order_by_mean = {c:np.median(genre_data[genre_data.Genre == c]['Revenue^{1/3}'])for c in genre_data['Genre'].unique()}
...
i = 0
for c in order:
    rev_c = genre_data[genre_data.Genre ==c]['Revenue^{1/3}']
    N = len(rev_c)
...
save_figure(plt, "Genre vs Revenue Transform")
