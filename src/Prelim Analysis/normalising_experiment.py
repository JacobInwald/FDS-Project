...
for c in data.columns:
    ...
    print(p[c])
    label[c] = f'{c} \n p = %.3f ∴ X~N(0,1)' % p[c] if p[c] > 0.05 else \
        f'{c} \n p <0.05, ∴ X≁N(0,1)'


# Plot data

fig, axs = plt.subplots(ncols=2, nrows=5, figsize=(5.5, 10), sharey=True)
fig.delaxes(axs[4][0])
fig.delaxes(axs[4][1])
fig.delaxes(axs[3][1])
...
for ax_r in axs:
    for ax in ax_r:
        ...
        pdf = scipy.stats.norm.pdf(
            x, loc=np.mean(data[c]), scale=np.std(data[c]))
        ax.hist(data[c], 20, label=label[c], color=next(
            colors)["color"], density=True)
        ax.plot(x, pdf)
        ax.set_xlabel('x')
